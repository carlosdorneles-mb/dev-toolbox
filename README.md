...
| `check-merged` | git | `git check-merged [--delete [--yes]]` - lista branches locais já mergeadas na raiz do remote (ancestor, patch-id via `cherry`, ou PR `state=MERGED` via `gh`). `--delete` apaga as encontradas - com `fzf` abre seleção múltipla, sem `fzf` cai pra confirmação y/N por branch. Ver [`git/check-merged/README.md`](git/check-merged/README.md). |
| `remote-branches` | git | `git remote-branches [org/repo\|URL] [--delete [--yes]] [--stale-days N] [--only-merged] [--only-stale] [--json] [--no-color]` - lista branches remotas de um repo GitHub via API `gh` (sem clone/fetch), com status de merge/PR, autoria e idade. `--delete` apaga as candidatas - com `fzf` abre seleção múltipla, sem `fzf` cai pra confirmação y/N por branch. Ver [`git/remote-branches/README.md`](git/remote-branches/README.md). |
